...
def load_file_list():
    global file_list
    # Get the list of files in the pframes folder ending with ".txt / .npz / .ply"
    file_list = [f for f in os.listdir('pframes') if f.endswith('.txt') or f.endswith('.npz') or f.endswith('.ply')]
    file_list.sort()
...
def load_frame(frame_idx):
    ...
    if pc_path.endswith('.txt'):
        point_cloud = np.loadtxt(pc_path, delimiter=' ')
    elif pc_path.endswith('.npz'):
        npz_data = np.load(pc_path)
        point_cloud = npz_data['point_cloud']
    elif pc_path.endswith('.ply'):
        with open(pc_path, 'rb') as f:
            header = f.readline().decode().strip()
            vertex_count = int(header.split()[2])
            data = np.fromfile(f, dtype=np.float32, count=vertex_count * 7)
            point_cloud = data.reshape(-1, 7)
...
def toggle_turntable():
    ...
    # Toggle rendering mode when turntable is active
    render_mode = ps_cloud.get_point_render_mode()
